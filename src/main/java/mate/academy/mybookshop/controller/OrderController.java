...
public class OrderController {
    ...
    @PostMapping
    @Operation(summary = "Create order", description = "Places order, moves books from cart to order")
    ...
    public List<OrderResponseDto> getAllOrders(@AuthenticationPrincipal UserEntity user, Pageable pageable) {
    ...
    @Operation(summary = "Get item from order", description = "Get specific item from specific order by id")
    ...
    public void changeStatus(@AuthenticationPrincipal UserEntity user,
                             @PathVariable Long id,
                             @Valid @RequestBody UpdateOrderRequestDto requestDto) {

        orderService.changeStatus(user, id, requestDto);
    }
}
